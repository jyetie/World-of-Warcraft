...
function Post:GetScanListAndSetup(GUIRef, options)
	...
	function HasEnoughToPost(item)
		local maxStackSize = select(8, TSMAPI:GetSafeItemInfo(item.itemString))
	...
	for itemString in pairs(tempList) do
		...
		elseif not HasEnoughToPost(item) then
			...
			if not TSM.Config:GetBoolConfigValue(groupName, "ignoreRandomEnchant", true) and searchTerm and searchTerm ~= "" then
...
function Post:ShouldPost(item)
	...
	if isInvalidSeller then
		TSM:Printf(L["Seller name of lowest auction for item %s was not returned from server. Skipping this item."], select(2, TSMAPI:GetSafeItemInfo(item.itemString)))
...
function Post:FindItemSlot(findItemString, allLocations, ignoreBagSlot)
	...
	for bag, slot, itemString in TSM:GetBagIterator() do
		...
		if findItemString == TSMAPI:GetSafeItemID(itemString) or findItemString == itemString then
...
function Post:GetNumInBags(itemString)
	local function GetItemBagCount(findItemString, noItemID)
		local num = 0
		for bag, slot, itemString in TSM:GetBagIterator() do
			local itemID = TSMAPI:GetItemString(TSMAPI:GetSafeItemID(itemString))
			if findItemString == itemID or findItemString == TSMAPI:GetSafeItemID(itemString) or findItemString == itemString then
...
function Post:DoAction()
	...
	if TSMAPI:GetSafeItemID(containerItemLink) ~= currentItem.itemString and TSMAPI:GetItemString(containerItemLink) ~= currentItem.itemString then
