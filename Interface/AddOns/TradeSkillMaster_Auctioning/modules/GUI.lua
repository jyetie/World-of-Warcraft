...
function private:CreateInfoText(parent)
	...
	frame.SetInfo = function(self, item)
		...
		elseif item.isReset then
			...
			local _,link,_,_,_,_,_,_,_,texture = TSMAPI:GetSafeItemInfo(item.itemString)
...
function private:CreateAuctionsST(parent)
	...
	rt:SetSort(7, true)
	rt:Hide()
	GUI:RegisterMessage("TSM_PRICE_PER_CHECKBOX_CHANGED", function() rt:SetColHeadText(6, GetPriceColName()) rt:RefreshRowData() end)
