...
function Crafting:CreateQueuingRegion() -- updated
	...
	for count=1, MAX_ROWS_QUEUING do
		...
		row:SetScript("OnDoubleClick", function(row, mouseButton) Crafting:OnCraftRowClicked(row, mouseButton, TSM.db.profile.doubleClick-1) end)
...
function Crafting:UpdateCrafting()
	...
	for index, data in ipairs(queueList) do
		...
		if not data.isTitle then
			...
			totalCost = totalCost + (cost or 0)*data.quantity*TSM.Data[Crafting.mode].crafts[data.itemID].numMade
			totalProfit = totalProfit + (profit or 0)*data.quantity*TSM.Data[Crafting.mode].crafts[data.itemID].numMade
		end
	end
...
function Crafting:UpdateShopping()
	if not Crafting.mode or not Crafting.shoppingRows then return end
...
function Crafting:UpdateQueuing()
	if not Crafting.mode or not Crafting.queuingRows then return end
