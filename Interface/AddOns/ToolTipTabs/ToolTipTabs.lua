...
local function BF()
	if LBF then
		if db.BF.SkinID and LBF:GetNormal(ToolTipTabs1) then
			return 1
		end
	else
		return nil
...
local tabTypes = {
	item = {name = "Item", order = 1, preset = "Rarity Colour",},
	spell = {name = "Spell", order = 2,},
	achievement = {name = "Achievement", order = 3, preset = "Completion Colour",},
	talent = {name = "Talent", order = 4,},
	quest = {name = "Quest", order = 5, preset = "Difficulty Colour",},
	enchant = {name = "Enchant", order = 6,},
	glyph = {name = "Glyph", order = 7,},
	instancelock = {name = "Instance Lock", order = 8},
	currency = {name = "Currency", order = 9, preset = "Rarity Colour"},
}
...
local function getIconColor(link, type)
	if db.colors[type].choice == 1 then
		if type == "currency" then
			local cr, cg, cb = GetItemQualityColor(select(8,GetCurrencyInfo(tonumber(link:match("^[^:]+:(%d+)")))) or 1) 
			return cr, cg, cb
...
local function getIcon(link, type)
	...
	elseif type == "currency" then
		return select(3,GetCurrencyInfo(tonumber(link:match("^[^:]+:(%d+)"))))		
	else
...
local function recolorTab(tab)
	...
	if BF() then
		if registryTooltip[registryTab[tab].tooltip].currentTab == tab then
			...
			LBF:GetNormal(registryTab[tab].obj):SetVertexColor(getIconColor(registryTab[tab].link, registryTab[tab].type))
		else
			...
			LBF:GetNormal(registryTab[tab].obj):SetVertexColor(cr*0.4, cg*0.4, cb*0.4)
		end
	else
		if registryTooltip[registryTab[tab].tooltip].currentTab == tab then
			registryTab[tab].obj.overlay:SetVertexColor(1.0, 1.0, 1.0)
...
function ToolTipTabs:OnInitialize()
		...
		args = {
			...
			appearance = {
				...
				args = {
					...
					format1 = {
						name = "\n\nBorder Colours (Requires Masque)", type = "description", order = 2,
	...
	for k, v in pairs(tabTypes) do
		self.options.args.appearance.args[k] = {
			...
			args = {
				choice = {
					name = "Colour Type", type = "select", order = 1,
					values = {[1] = v.preset or "No Preset Available", [2] = "ButtonFacade Colour", [3] = "Custom",},
				},
				custom = {
					name = "Custom Colour", type = "color", order = 2,
...
function ToolTipTabs:OnEnable()
	if LBF then
		...
		LBF:Register("ToolTipTabs", function(argAndArg, Group, SkinID, Gloss, Backdrop, Colors)
			db.BF.SkinID = SkinID
			db.BF.Gloss = Gloss
