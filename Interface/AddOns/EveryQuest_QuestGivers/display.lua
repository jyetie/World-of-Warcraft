...
local QuestGivers = EveryQuest:GetModule("QuestGivers")
local Display = EveryQuest:NewModule("Display","AceEvent-3.0")
...
local function recyclePin(pin)
	...
	-- pin.questCount = nil
	-- pin.title = nil
	-- if type(pin.data) == "table" then
		-- pin.data = del(pin.data)
	-- end
	pin.worldmap = false
	-- pin.nodeID = 0
	pin.keep = nil
...
local function deletePin(button,pin)
	local count = 0
	for uid, zone in pairs(pin.data[3]) do
		if QuestGivers.viewCache[uid] then
			EveryQuest.dbpc.profile.ignored[uid] = true
			count = count + 1
		end
	end
	EveryQuest:Print( format( L["Ignored %d quests"], count) )
	-- QuestGivers:RemoveNodeByID(pin.zone, pin.nodeType, pin.coords)
	-- Display:SendMessage("QuestGiversDisplayChanged")
	QuestGivers:DisplayChanged()
...
local function showPin(self)
	if (self.data) then
		...
		tooltip:AddLine( format("|cffffff00%s%s|r", self.data[1] == 3 and COMBATLOG_HIGHLIGHT_KILL .. ": " or "", self.data[2]), 1, 1, 0 )
		for uid, zone in pairs(self.data[3]) do
			if QuestGivers.viewCache[uid] then
				tooltip:AddLine( QuestGivers:GetQuestDisplayText(uid, zone))
			end
		end

		for id, pin in pairs(pinset) do
			if pin:IsMouseOver() and pin.data and pin ~= self then
				tooltip:AddLine( format("|cffffff00%s%s|r", pin.data[1] == 3 and COMBATLOG_HIGHLIGHT_KILL .. ": " or "", pin.data[2]), 1, 1, 0 )
				for uid, zone in pairs(pin.data[3]) do
					if QuestGivers.viewCache[uid] then
						tooltip:AddLine( QuestGivers:GetQuestDisplayText(uid, zone))
					end
...
local function pinClick(self, button)
	if button == "RightButton" and self.worldmap then
		pinClickedOn = self
		ToggleDropDownMenu(1, nil, EveryQuest_GenericDropDownMenu, self:GetName(), 0, 0)
...
local function addTomTomWaypoint(button,pin)
	if TomTom then
		...
		local x, y, level = QuestGivers.mapData:DecodeLoc(pin.coords)
		TomTom:AddZWaypoint(c, z, x*100, y*100, pin.data[2], nil, true, true)
	end
end
--[[
...
local function generatePinMenu(self,level)
	...
	if (level == 1) then
		...
		info.isTitle      = 1
		info.text         = L["QuestGivers Pin Options"]
		info.notCheckable = 1
		...
		for id, pin in pairs(worldmapPins) do
			if pin:IsMouseOver() and pin.data then
				info.text = format( L["Ignore quests given by %s"], pin.data[2])
				info.icon = QuestGivers:GetTexture(pin.coords) or nodeTextures['normal']
				info.func = deletePin
				-- info.notCheckable = 1
				info.arg1 = pin
				UIDropDownMenu_AddButton(info, level);
...
local EveryQuest_GenericDropDownMenu = CreateFrame("Frame", "EveryQuest_GenericDropDownMenu")
EveryQuest_GenericDropDownMenu.displayMode = "MENU"
EveryQuest_GenericDropDownMenu.initialize = generatePinMenu
local last_update = 0
...
function Display:OnEnable()
	db = QuestGivers.db.profile

	trackingCircle = QuestGivers.trackingCircle
	EveryQuest:Debug(trackingCircle)
	nodeTextures = QuestGivers.nodeTextures
...
function Display:RegisterMapEvents()
	...
	self:RegisterMessage("QuestGiversConfigChanged", "ConfigChanged")
	self:RegisterMessage("QuestGiversDisplayChanged", "DisplayChanged")
	self:RegisterMessage("EveryQuestDisplayChanged", "DisplayChanged")
...
function Display:UnregisterMapEvents()
	...
	self:UnregisterMessage("QuestGiversConfigChanged")
	self:UnregisterMessage("QuestGiversDisplayChanged")
	self:UnregisterMessage("EveryQuestDisplayChanged")
...
function Display:MINIMAP_UPDATE_TRACKING()
	...
	for id=1, count do
		...
		if texture == "Interface\\Minimap\\Tracking\\TrivialQuests" and active then
			active_tracking = true
		else
			if texture == "Interface\\Minimap\\Tracking\\TrivialQuests" and not active then
...
function Display:UpdateVisibility()
	...
	local visible = false
	local state = db.showWorldMap
	if state == "always" then
		visible = true
	elseif state == "active" then
		visible = active_tracking == true
	end
	QuestGivers.Visible = visible
	
	local visible = false
	local state = db.showMinimap
	if state == "always" then
		visible = true
	elseif state == "active" then
		visible = active_tracking == true
	end
	QuestGivers.MMVisible = visible
...
function Display:DisplayChanged()
	self:UpdateVisibility()
	self:UpdateMaps()
end
--[[
...
function Display:getMapPin()
	...
	-- create a new pin
	pinCount = pinCount + 1
	pin = CreateFrame("Button", "QuestGiversPin"..pinCount, WorldMapButton)
	pin:SetFrameLevel(5)
...
function Display:addWorldPin(coord, data, zone, index)
	local pin = worldmapPins[index]
	if not pin then
		pin = self:getMapPin()
		local x, y, level = QuestGivers.mapData:DecodeLoc(coord)
		pin.coords = coord
		pin.data = data
		-- pin.npcName = npcName
		-- pin.questCount = #data
		...
		pin:SetHeight(12 * db.icon_scale)
		pin:SetWidth(12 * db.icon_scale)
		pin:SetAlpha(db.icon_alpha)
		pin:EnableMouse(true)
		pin.texture:SetTexture(QuestGivers:GetTexture(coord) or nodeTextures['normal'])
		-- pin.texture:SetTexture(QuestGivers.nodeTextures['normal'])
...
function Display:getMiniPin(coord, data, zone, index)
	...
	if not pin then
		...
		pin:SetHeight(12 * db.mmicon_scale / minimapScale)
		pin:SetWidth(12 * db.mmicon_scale / minimapScale)
		--pin:SetAlpha(db.alpha)
		pin:EnableMouse(db.minimapTooltips)
		pin.isCircle = false
		pin.texture:SetTexture(QuestGivers:GetTexture(coord))
		pin.texture:SetTexCoord(0, 1, 0, 1)
		pin.texture:SetVertexColor(1, 1, 1, 1)
		pin.x, pin.y, pin.level = QuestGivers.mapData:DecodeLoc(coord)
		pin.x1, pin.y1 = QuestGivers.mapData:PointToYards(zone,pin.level,pin.x,pin.y)
		-- pin.x1, pin.y1 = QuestGivers:PointToYards(pin.x,pin.y,zone,pin.level)
...
function Display:addMiniPin(pin, refresh)
	...
	if (not pin.isCircle or refresh) and trackShow and dist_2 <= db.trackDistance^2 then
		-- EveryQuest:Debug(trackingCircle)
		pin.texture:SetTexture(trackingCircle)
		pin.texture:SetVertexColor(unpack(db.trackColor))
		pin:SetHeight(12 * db.mmicon_scale / minimapScale)
		pin:SetWidth(12 * db.mmicon_scale / minimapScale)
		pin.isCircle = true
		pin.texture:SetTexCoord(0, 1, 0, 1)
	...
	elseif (pin.isCircle or refresh) and dist_2 > db.trackDistance^2 then
		pin:SetHeight(12 * db.mmicon_scale / minimapScale)
		pin:SetWidth(12 * db.mmicon_scale / minimapScale)
		pin.texture:SetTexture(QuestGivers:GetTexture(pin.coords) or nodeTextures['normal'])
		pin.texture:SetVertexColor(1, 1, 1, 1)
		pin.texture:SetTexCoord(0, 1, 0, 1)
		pin.isCircle = false
	end
	...
	if db.nodeRange or alpha >= 1 then
		...
		pin:SetAlpha(min(alpha,db.mmicon_alpha))
	else
		pin:Hide()
	end
end
...
function Display:UpdateIconPositions()
	...
	if x ~= lastX or y ~= lastY or facing ~= lastFacing or level ~= lastLevel or refresh then
		...
		mapRadius = QuestGivers.minimap_size[indoors][zoom] / 2

		-- we calculate the distance to the node in yards
		local _x, _y =  QuestGivers.mapData:PointToYards(zone,level,x,y)
...
function Display:UpdateMiniMap(force)
	...
	-- EveryQuest:Debug(format("Display:UpdateMiniMap(%s)", tostring(force)))
	-- update our zone info
	zone = GetCurrentMapAreaID()
	if QuestGivers.map_phasing[zone] then zone = QuestGivers.map_phasing[zone] end
	local level = GetCurrentMapDungeonLevel()
	...
	if x ~= lastX or y ~= lastY or diffZoom or facing ~= lastFacing or level ~= lastLevel or force then
		...
		minimapShape = GetMinimapShape and QuestGivers.minimap_shapes[GetMinimapShape() or "ROUND"]
		mapRadius = QuestGivers.minimap_size[indoors][zoom] / 2
		...
		-- for i,db_type in pairs(GatherMate.db_types) do
			if QuestGivers.MMVisible then
				for coord, data in QuestGivers:FindNearbyNode(zone, x, y, level, mapRadius*nodeRange, ignoreFilter) do
					-- local pin = self:getMiniPin(coord, nodeID, db_type, zone, (i * 1e14) + coord)
					local pin = self:getMiniPin(coord, data, zone, coord)
					pin.keep = true
...
function Display:UpdateWorldMap(force)
	...
	if QuestGivers.map_phasing[zoneid] then zoneid = QuestGivers.map_phasing[zoneid] end
	...
	-- for i,db_type in pairs(GatherMate.db_types) do
		if QuestGivers.Visible then
			for coord, data in QuestGivers:GetNodesForZone(zoneid, ignoreFilter) do
				local nx,ny,nlevel = QuestGivers.mapData:DecodeLoc(coord)
				if nlevel == mapLevel then
					-- self:addWorldPin(coord, nodeID, "normal", zoneid, (i * 1e14) + coord).keep = true
					self:addWorldPin(coord, data, zoneid, coord).keep = true
				end
			end
			-- del(data)
		end
	...
	if lastScale ~= db.icon_scale or lastAlphaPref ~= db.icon_alpha then
		local scale, alpha = db.icon_scale, db.icon_alpha
